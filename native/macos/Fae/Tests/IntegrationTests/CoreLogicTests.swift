...
final class ToolPermissionSnapshotTests: XCTestCase {
    ...
    func testCanvasHTMLContainsTriggerText() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("test trigger"))
    }

    func testCanvasHTMLContainsToolMode() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("full"))
    }

    func testCanvasHTMLShowsGrantedBadges() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("<span class='ok'>granted</span>"))
    }

    func testCanvasHTMLShowsNotGrantedBadges() {
        let perms = PermissionStatusProvider.Snapshot(
            microphone: false,
            contacts: true,
            calendar: true,
        ...
        let snapshot = makeSnapshot(permissions: perms)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("<span class='warn'>not granted</span>"))
    }

    func testCanvasHTMLShowsAllowedTools() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("<code>read</code>"))
        XCTAssertTrue(html.contains("<code>write</code>"))
    }

    func testCanvasHTMLShowsDeniedTools() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("<code>bash</code>"))
        XCTAssertTrue(html.contains("Not available in this mode"))
    }

    func testCanvasHTMLShowsNoneWhenNoAllowedTools() {
        ...
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("<li>None</li>"))
    }

    func testCanvasHTMLShowsAllGrantedMessageWhenNoMissingPerms() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("All tracked permissions are currently granted"))
    }

    func testCanvasHTMLIncludesPermissionGrantChips() {
        let perms = PermissionStatusProvider.Snapshot(
            microphone: false,
            contacts: true,
            calendar: true,
        ...
        let snapshot = makeSnapshot(permissions: perms)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("request_permission?capability=microphone"))
    }

    func testCanvasHTMLReflectsThinkingToggle() {
        // thinkingEnabled = true should show "Thinking OFF" toggle
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("Thinking ON") || html.contains("Thinking OFF"))
    }

    func testCanvasHTMLReflectsOwnerGateState() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
        XCTAssertTrue(html.contains("enabled"))
    }

    func testCanvasHTMLIsValidHTMLStructure() {
        let snapshot = makeSnapshot(permissions: allGrantedPermissions)
        let html = snapshot.toCanvasHTML()
