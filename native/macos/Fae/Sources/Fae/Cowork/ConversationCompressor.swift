...
actor ConversationCompressor: Sendable {
    ...
    func compressIfNeeded(
        ...
        // For now, return a placeholder that will be replaced with LLM response
        // Task 2 implements async generateSummary() which will be called from Task 3
        let summaryMessage = WorkWithFaeConversationMessage(
            id: UUID(),
            role: "summary",
            content: summaryPrompt,  // Store prompt as placeholder
            timestamp: Date(),
            modelID: modelID,
