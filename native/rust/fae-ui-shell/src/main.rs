// Tests may use `.expect()`/`.unwrap()`/`panic!` per the project convention
// (production code stays panic-free). The orb-shell CI lints `--all-targets`
// with strict panic/unwrap/expect denies, which also covers `#[cfg(test)]` code,
// so scope those allows to test builds only.
