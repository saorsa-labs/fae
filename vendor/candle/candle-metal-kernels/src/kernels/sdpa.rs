...
pub fn call_sdpa_full(
    ...
    // (callers build a full causal / sliding-window mask), so the in-kernel
    // `do_causal` path is redundant. Worse, combining `do_causal` with a mask
    // drives the steel_attention kernel's causal `kb_lim` block-truncation +
    // triangle masking into a state where a query row can end up fully masked
    // at specific (periodic) sequence lengths, yielding sum(exp)=0 -> 0/0 = NaN
    // logits (Gemma-4 on Metal; mistral.rs #2214). Letting the mask carry
    // causality avoids that path with no correctness loss and no measurable
    // perf change (the upper-triangle skip is not the prefill bottleneck).
