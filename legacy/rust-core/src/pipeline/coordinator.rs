...
async fn run_llm_stage(
    ...
    'outer: loop {
                ...
                match input {
                    ...
                    Input::BackgroundResult(result) => {
                        ...
                        if !result.spoken_summary.trim().is_empty() {
                            ...
                            // Clear any barge-in interrupt flag before narrating the result.
                            // Barge-in fires whenever `assistant_generating` is true, so any
                            // audio above the threshold while the agent was running sets
                            // `interrupt = true` — causing TTS to silently drop the result.
                            // The background result is always fresh content the user hasn't
                            // heard, so it must be spoken regardless of what happened during
        ...
        //
        // `classify_intent_with_context` also upgrades short confirmations
        // ("yes", "go ahead") to background-agent routing when Fae's previous
        // response mentioned a tool-backed action she offered to take.
        let last_assistant_text = conversation_turns
            .last()
            .map(|t| t.assistant_text.as_str())
            .unwrap_or("");
        let intent =
            crate::agent::classify_intent_with_context(&user_text, last_assistant_text);
        if intent.needs_tools {
            ...
            // Clear any barge-in interrupt flag before sending the canned ack.
            // When barge-in fires during Fae's previous response, `interrupt` is
            // set to true and stays true until `generate_response` resets it.
            // Since background-agent routing skips `generate_response`, the flag
            // would silently cause TTS to drop both the canned ack and the final
            // background-agent result, leaving the user with no audio response.
            interrupt.store(false, Ordering::Relaxed);
        ...
        for bg_result in pending_bg_results {
            ...
            // Reset interrupt so barge-in during the agent's run doesn't silently drop this.
            interrupt.store(false, Ordering::Relaxed);
            let _ = tx
                .send(SentenceChunk {
        ...
        if chunk.is_final {
            match mode {
                Mode::Json => {
                    ...
                    // If canvas render succeeded, use its description.
                    // Otherwise classify the JSON to produce an honest spoken response:
                    //   - Tool-call JSON (has "name"/"function" key): the voice LLM tried to
                    //     call a tool it doesn't have — tell the user what was attempted.
                    //   - Unknown JSON: pass nothing through (silent, avoids confusing TTS).
                    let output_text = if let Some(description) = spoken {
                        Some(description)
                    } else {
                        warn!(
                            "non-canvas JSON in voice stream: {}",
                            &cleaned[..cleaned.len().min(200)]
                        );
                        // Try to extract a "name" field to describe what was attempted.
                        let attempted_tool = json_str.and_then(|js| {
                            // Quick scan for "name": "..." in the JSON string.
                            let name_key = "\"name\"";
                            js.find(name_key).and_then(|pos| {
                                let after = js[pos + name_key.len()..].trim_start();
                                let after = after.trim_start_matches(':').trim_start();
                                if after.starts_with('"') {
                                    let after = &after[1..];
                                    after.find('"').map(|end| after[..end].to_owned())
                                } else {
                                    None
                                }
                            })
                        });
                        attempted_tool.map(|tool| {
                            // Convert snake_case tool name to readable words.
                            let readable = tool.replace('_', " ");
                            format!("I tried to {readable}, but I don't have the right tools available for that right now.")
                        })
                    };
