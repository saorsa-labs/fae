...
/// Classify intent, falling back to conversation context when the user confirms
/// a prior tool action.
///
/// When `user_text` is a short affirmative phrase ("yes", "go ahead", etc.) and
/// `last_assistant_text` shows Fae was discussing a scheduling/tool action, the
/// response is upgraded to background-agent routing so the action is actually
/// executed via tools rather than hallucinated in the tool-less voice channel.
pub fn classify_intent_with_context(
    user_text: &str,
    last_assistant_text: &str,
) -> IntentClassification {
    let base = classify_intent(user_text);
    if base.needs_tools {
        return base;
    }

    // Only upgrade short confirmations — longer utterances are likely new requests.
    let lower = user_text.to_ascii_lowercase();
    let word_count = lower.split_whitespace().count();
    if word_count > 6 || !contains_any(&lower, intent::CONFIRMATION_KEYWORDS) {
        return base;
    }

    // Check whether Fae's last response described a tool-backed action she
    // offered to take, so the user's "yes" is confirming that action.
    let prev_lower = last_assistant_text.to_ascii_lowercase();
    let had_scheduler = contains_any(&prev_lower, intent::SCHEDULER_KEYWORDS);
    let had_reminders = contains_any(&prev_lower, intent::REMINDERS_KEYWORDS);
    let had_calendar = contains_any(&prev_lower, intent::CALENDAR_KEYWORDS);

    if !had_scheduler && !had_reminders && !had_calendar {
        return base;
    }

    // Build an allowlist appropriate for the prior context.
    let tool_allowlist = if had_scheduler {
        vec![
            "list_scheduled_tasks".to_owned(),
            "create_scheduled_task".to_owned(),
            "update_scheduled_task".to_owned(),
            "delete_scheduled_task".to_owned(),
            "trigger_scheduled_task".to_owned(),
        ]
    } else if had_reminders {
        vec![
            "list_reminder_lists".to_owned(),
            "list_reminders".to_owned(),
            "create_reminder".to_owned(),
            "set_reminder_completed".to_owned(),
        ]
    } else {
        vec![
            "list_calendars".to_owned(),
    ...
    let task_description = format!(
        "The user confirmed: \"{user_text}\"\n\
         Prior assistant context: {last_assistant_text}\n\
         Complete the action the user agreed to using your available tools. \
         Provide a concise spoken confirmation when done."
    );
