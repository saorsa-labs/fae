//! Vendored from github.com/srv1n/cbx — adapted for ort 2.0.0-rc.11.
#![allow(
    dead_code,
...
use ort::value::{DynValue, Outlet, Tensor, ValueType};
use tokenizers::Tokenizer;

use super::audio;
use super::audio::TARGET_SAMPLE_RATE;
use super::download;
use super::voice::{TensorF32, TensorI64, VoiceProfile};
...
fn elem_type_of(outlet: &Outlet) -> Option<ElemType> {
...
impl Chatterbox {
    pub fn load(paths: &download::ChatterboxPaths) -> Result<Self> {
        Self::load_with(paths, &SessionConfig::default())
    }

    pub fn load_with(paths: &download::ChatterboxPaths, cfg: &SessionConfig) -> Result<Self> {
...
fn session_builder(cfg: &SessionConfig) -> Result<ort::session::builder::SessionBuilder> {
    ...
    fn coreml_ep(cfg: &SessionConfig) -> ort::execution_providers::ExecutionProviderDispatch {
        use ort::execution_providers::coreml::{
            ComputeUnits, CoreML as CoreMLEp, ModelFormat, SpecializationStrategy,
        };
        let mut coreml = CoreMLEp::default()
...
fn inspect_kv_cache_io(
    ...
    for i in language_model.inputs() {
        ...
        past_kv_names.push(i.name().to_owned());
    ...
    for o in language_model.outputs() {
        if o.name().starts_with("present.") {
            ...
            if past_kv_names.iter().any(|n| n == &past) {
                present_to_past.insert(o.name().to_owned(), past);
            }
            continue;
        }
