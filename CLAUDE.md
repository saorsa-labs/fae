...
6. **LLM** — Qwen3.5 with native tool calling; max 5 tool turns per query
...
| `ML/MLXLLMEngine.swift` | Qwen3.5 LLM inference via mlx-swift |
...
| `SettingsModelsPerformanceTab.swift` | Model selection, performance tuning, voice settings |
...
| `SettingsDiagnosticsTab.swift` | Runtime diagnostics: model loaded state, pipeline status, restart counts |
