...
## Symphony sidecar (owner-opt-in)
`FAE_SYMPHONY_ENABLED=1` makes the daemon spawn and **supervise** `fae-symphony-runner` as a child process — this Fae joins a group-of-Fae work queue (claims x0xd TaskItems, executes them through this daemon's `conversation.delegate`, publishes signed handoffs). **OFF by default**: joining a swarm is a deliberate owner choice. Requires the runner's task config (`FAE_SYMPHONY_TASK_LIST` + `FAE_SYMPHONY_WORKSPACE_ROOT`, or a `FAE_SYMPHONY_CONFIG` TOML — full variable list in `fae-symphony-runner/README.md`); the daemon injects its own `FAE_DAEMON_SOCKET`/`FAE_DAEMON_TOKEN_PATH` and passes `FAE_SYMPHONY_*` through the scrubbed child env (provider secrets never cross; secret-marked names are refused even under the passthrough prefix). Binary lookup: `FAE_SYMPHONY_RUNNER_BIN` → beside `fae-daemon` → `PATH`. Lifecycle: crash ⇒ exponential-backoff restart (max 5 consecutive, 1 s→30 s; a run ≥60 s resets the budget, then one loud give-up), daemon shutdown/parent death ⇒ child killed (fae-engine sidecar registry, same reaping as llama-server). Misconfiguration is loud but never blocks daemon startup. The daemon still links **no** x0x-symphony crate — the quarantine invariant holds.
