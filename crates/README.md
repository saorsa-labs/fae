...
| `fae-daemon` | Daemon binary. Chunk 1 = control-plane bootstrap (run dir `0700`, token `0600`) + demonstrated `authorize → audit`. | **Chunk 1 skeleton ✅** |
...
- **Chunk 2 — network surface:** Unix-socket (default) + TCP-loopback (diagnostic) listener; `Host`/`Origin` enforcement; **single-use 60 s WS/SSE stream tickets** (no `?token=`); per-message `authorize()`; peer input only via `fae-envelope-gate`. ADR-002 command/event protocol v2.
