...
4. **Override the model mode (`ModelMode`).** The mode (`pure-local` / `local-symphony` / `all-available`) is the single most powerful egress control — the mode cap gates every non-local route (§5.2) and, under `pure-local`, prevents cloud egress from **any** daemon surface regardless of recipe. MetaOpt must not be able to mutate it. Today this is **structurally** guaranteed: `ModelMode` lives on `ConductorEgress` (the runtime), **not** on `FaeConductorRecipe` (the mutation surface), so no recipe mutation can touch it. It is stated explicitly here — and declared **outside the `ConfigKnob` surface too** — because model-mode is an egress-safety control, not a tunable knob; a future change must not reclassify it as a MetaOpt-mutable config value. (The same lesson as the M2 NOTE-2 review: the surface everyone assumes is out-of-reach — model-mode here, `agent.session_start` there — must be named explicitly so it is not later "rediscovered" as reachable.)
...
**Layer 2 — Runtime layer (authoritative, in the conductor executor).** This is the layer that actually owns permission. The M2 executor's **§5 gate pipeline re-asserts `mode-cap → secret/credential-membrane → budget → approval` on every turn, regardless of recipe content.** The recipe is **advisory**; the runtime gates are **authoritative**. So even a maliciously or erroneously mutated recipe **cannot bypass egress protection**, because the gates run in a load-bearing order:
- The **mode cap runs first** (§5.2): a recipe that claims a cloud lane still cannot egress under `pure-local` — the mode owns this, not the recipe.
- The **secret/credential membrane runs before any cloud-bound request is constructed** (M2 §5.3 runs before §5.6). It blocks **secrets/credentials** (API keys, tokens, private keys, seed phrases, password assignments, OTPs); it does **not** detect personal content (health, address, location, family, finances) — see `docs/architecture/egress-scope-and-stage3-hold-2026-06-23.md`. The defense-in-depth here therefore guarantees a mutated recipe cannot cause **secret** leakage or **unauthorized** egress; the scope of what the membrane catches is a separate (Stage 3) posture question, not a recipe-mutation question.
...
- `docs/architecture/conductor-m2-reward-eval-shadow-routing-spec-2026-06-23.md` §5 — the per-role-call executor gate pipeline (mode-cap → secret/credential-membrane → budget → approval → worker → record → telemetry); §5.6 "constructs the provider request after gates 5.2–5.5 pass" (the load-bearing invariant)
- `docs/architecture/egress-scope-and-stage3-hold-2026-06-23.md` — the membrane's real coverage (credential/secret filter, not PII) and why the Stage 3 default flip is held

> **v2 (2026-06-23, advisor protected-kernel review):** added `ModelMode` to the enforceable-constraints (§Enforceable constraints #4) — structurally protected today (lives on `ConductorEgress`, not the recipe) but now stated explicitly and declared outside the `ConfigKnob` surface too. Corrected the "PII membrane" naming to "secret/credential membrane" throughout; the defense-in-depth argument is reframed as preventing **secret** leakage + **unauthorized** egress (the membrane's credential-only scope is a Stage 3 posture question, not a recipe-mutation question, so the argument holds).
