...
### Security (HIGH)
- SessionStore redacts at the `appendMessage` choke point (defense-in-depth `SensitiveContentPolicy` + `SensitiveDataRedactor`) and the dead `private_key_block` PEM regex is corrected. The redaction chain covers AWS `AKIA…` IDs (`aws_access_key` rule) and URL-embedded credentials (basic-auth userinfo + `?token=`/`?api_key=` values), so `session_search` coverage matches the withholding path.
