...
> **Revision:** **Rev 14** (2026-06-11) — owner clarification: **"mistral everywhere" means the LLM lane, not the whole perception/personal-training stack.** Primary LLM inference moves to mistral.rs for cross-platform Rust unification, but **MLX survives where it is currently irreplaceable**: the personal LoRA training loop (`mlx-lm`/`mlx-tune`), Kokoro-adjacent Apple voice work, STT until Gemma-4-audio parity is product-proven, and VLM short-term. Personal adapters trained on MLX must pass an **MLX → PEFT/safetensors → mistral.rs LoRA** compatibility spike before they are assumed loadable. `llama.cpp` remains mandatory for Vulkan acceleration on consumer AMD/Intel Windows/Linux because mistral.rs documents CUDA/Metal/CPU performance paths, not Vulkan. Full pipeline follow-up: `docs/architecture/full-cross-platform-ml-pipeline-2026-06-11.md`.
...
2. **LLM engine swappable behind one adapter.** Primary **mistral.rs** in-process; fallback **`llama-server`** (GGUF) behind the same adapter; optional **MLX-turbo on Apple** remains future/premium. **llama.cpp has no MLX backend** — the turbo path is MLX-direct/Ollama, per-platform behind the same adapter. **Do not over-read this as "mistral replaces MLX everywhere": mistral.rs owns the LLM inference lane; MLX remains in the personal-training/perception lanes until a spike proves cross-platform replacements are equal.**
...
4. **Self-learning is application-layer and cross-platform by default, with weight-learning retained as Fae's moat.** Fae already matches Hermes Agent's loop (skills/memory/session-search/user-model), no MLX/training required for v1. But **personal LoRA training is not dropped**: MLX (`mlx-lm`/`mlx-tune`) remains the Apple-first training lane because it already fine-tunes quantized models and writes adapter artifacts; Unsloth/PEFT/Axolotl/torchtune are candidate non-Apple lanes, not proven replacements. To exceed: a dialectic/theory-of-mind memory upgrade (Honcho is the exemplar but **AGPL** — see §13).
...
|---|---|---|---|
| **`llama-server` (GGUF)** | all | sidecar (HTTP) or `llama-cpp-2` FFI (v0.1.146) | **Fallback to prove in G2 and mandatory Vulkan lane.** OpenAI/Anthropic HTTP, `--jinja` tools, GGUF LoRA hot-swap. CUDA strongest; Vulkan makes it the practical GPU path for consumer AMD/Intel Windows/Linux where mistral.rs is CPU-only. **Verified Gemma-4 audio-in.** |
| **MLX-direct (turbo/training/perception)** | Apple | sidecar or native Swift/Python tooling | Optional Apple premium inference path, but **required short-term for personal LoRA training (`mlx-lm`/`mlx-tune`), VLM/STT gaps, and Apple-optimized perception work** until parity spikes pass. S10/S14. |
| **mistral.rs (candle)** | all | **in-process pure-Rust crate** | **CHOSEN for the LLM lane (S13-confirmed).** Gemma-4 E4B on Metal ~65 tok/s, no #2051 hang, **tool calling ✓, unified audio STT ✓** (in-process). Prior art in `legacy/rust-core/` (0.7). Risks: pre-1.0 API churn, single-maintainer concentration around EricLBuehler, no Vulkan backend, adapter-conversion unproven. See §8a and S14. |
