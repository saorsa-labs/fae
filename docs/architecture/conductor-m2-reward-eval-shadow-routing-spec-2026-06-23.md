...
### Stage 3 — Separate gated cutover to `all-available` default — **HELD**

> **⚠ ON HOLD (2026-06-23, advisor-corrected):** the "all-available default" destination was premised on the membrane "protecting users' data." Verified in source, the membrane catches **secrets/credentials only, not personal content** (health, address, location, family, finances, relationships). Default-on cloud egress for a privacy-first companion (ADR-001/003/007: "all intelligence runs locally… no data leaves the device") would silently ship deeply personal non-secret content to third parties, with no consent UX. **Stage 3 is held** until the owner re-decides the default privacy posture with this corrected premise. Recommended default: `pure-local` or `local-symphony`, with `all-available` an explicit disclosed opt-in. Full analysis: [`egress-scope-and-stage3-hold-2026-06-23.md`](egress-scope-and-stage3-hold-2026-06-23.md).

Only after Stage 1 is merged AND Stage 2 clears AND the owner has made the conscious privacy-posture call above: a **separate** change flips the `FAE_MODEL_MODE` default from `pure-local` to `all-available`, gated by the **release-validation contract** (`docs/checklists/app-release-validation.md` + `docs/checklists/main-and-cowork-live-test-scenarios.md`) **and a first-run cloud-egress disclosure UX** (does not exist today; required real work if the default flips). This is its own commit, its own review, its own gate — never bundled with the wiring landing. Until Stage 3, cloud egress is opt-in (`FAE_MODEL_MODE=all-available` or `local-symphony`), not default.

**Stage 3 prerequisites (revised, owner decision 2026-06-23):** real provider pricing is **NOT** required — conductor pricing is non-authoritative; provider-side spend caps own real cost (§5.4). NOTE-2 (agent-command egress gating) is **CLOSED/merged** — both daemon cloud-egress surfaces are now gated. The remaining gates are (a) the **owner's privacy-posture decision** (this section, held), (b) the **disclosure UX** if the default flips, and (c) the release-validation contract.
