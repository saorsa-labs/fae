...
- **Status:** DRAFT v2 — for G-M2-spec re-review. v1 CONDITIONAL FAIL (no BLOCKER, 5 MAJORs M1–M5, 6 MINORs, 2 NOTEs). All folded in here: M1 (§2 signatures corrected to as-landed code), M2 (§5.4/§5.5 LocalOnly no-op + §6 test for all-available-local byte-identity), M3 (§5.4 CostEstimate provenance + §10 pricing module), M4 (§7 user_signal late-arriving feedback append + aggregator join), M5 (§3.1 per-provider retention explicitly deferred). MINORs: Tier-C reconciliation (§4), approval-denial copy (§11), no-phantom-spend test (§13), reward-window vs corpus-score distinction (§7/§8), M1 executor UnexpectedApproval modification (§10), ActualCost-on-error (§5.7). NOTEs: Reward type (§7), per-worker-budget-is-new-code phrasing (§3.1).
- **Date:** 2026-06-23
...
- `estimate_cost(worker_id, prompt, max_output_tokens) -> CostEstimate`: input tokens estimated by a conservative heuristic (≈ chars/4, deliberately over-counting so the cap errs toward blocking); output tokens estimated from `max_output_tokens` (worst case). Wall-clock is left at `0` (no per-call latency estimate in M2 — the wall-clock cap is enforced by the call itself, not the estimate).
