...
### 5.3 Secret/credential membrane (NEW — the F-2 egress authority landing)

> **⚠ Scope correction (2026-06-23):** despite the historical name "PII membrane," this authority detects **secrets/credentials only** (API keys, tokens, private keys, seed phrases, password assignments, OTPs) — see [`egress-scope-and-stage3-hold-2026-06-23.md`](egress-scope-and-stage3-hold-2026-06-23.md) §1 for the full rule table. It does **not** detect personal content (health, address, location, family names, finances, relationships). It prevents **secret** leakage and **unauthorized** egress (under `pure-local`); it does not prevent personal-content egress under an operator-enabled cloud mode. That posture is a Stage 3 decision, now **held** pending owner re-decision (§15).
...
- **Owner decision 2026-06-23 — pricing is OPTIONAL / non-authoritative.** Provider-side spend caps (OpenAI/Anthropic/etc.) are the authoritative cost control — stricter and more correct than a conductor-maintained heuristic, and offloading spend there removes the staleness/miscalculation risk of a local pricing table. Conductor cost gating is a secondary, opt-in layer: `FAE_PROVIDER_PRICING` + the `budget.rs` cost dimensions remain available, but the conductor makes **no spend guarantee** and real pricing is **NOT** a Stage 3 prerequisite (§15). The mock `1/1` startup defaults in `main.rs` are non-authoritative test scaffolding, retained so the §5.4 path is exercisable without live billing — they are not a billing promise. (Gap of record: provider caps are per-provider and do not aggregate conductor chain orchestration spend across calls/providers; accepted as revisitable.)
...
### Stage 3 — Separate gated cutover to `all-available` default — **DECIDED: not proceeding; all-available stays opt-in**

> **✅ DECIDED 2026-06-23 (owner acknowledgment):** the security architecture is **Fae-as-local-coordinator** — Gemma is local, private, the boss; she coordinates ACP harnesses + cloud API models as tools; cloud coordination is **Fae-mediated delegated work** (scoped tasks she chooses to send), not a data firehose; the membrane is **defense-in-depth for credentials** (catches accidental secret leaks), not the trust boundary. The architecture is sound. So the Stage 3 question is *not* "is cloud egress safe" — it is *"what default coordination reach fits a privacy-first companion?"* Answer: **`pure-local` default; `all-available` explicit opt-in.** Matches ADR-001/003/007's local-first identity ("all intelligence runs locally… no data leaves the device"). The Stage 3 flip-to-default is **not proceeding.** Full security model + analysis: [`egress-scope-and-stage3-hold-2026-06-23.md`](egress-scope-and-stage3-hold-2026-06-23.md) §0–§2.

Cloud egress therefore stays **opt-in** (`FAE_MODEL_MODE=all-available` or `local-symphony`), not default — indefinitely, until/unless a future owner decision reverses this with a fresh disclosure-UX commitment. If that ever happens, it remains a **separate** change gated by the **release-validation contract** + a **first-run cloud-egress disclosure UX** (does not exist today; required real work if a default flip is ever re-proposed). Never bundled with the wiring landing.

**Stage 3 prerequisites (revised, owner decision 2026-06-23):** real provider pricing is **NOT** required — conductor pricing is non-authoritative; provider-side spend caps own real cost (§5.4). NOTE-2 (agent-command egress gating) is **CLOSED/merged** — both daemon cloud-egress surfaces are now gated. **The Stage 3 default-flip is not proceeding (owner decision).** The only remaining `all-available` work is the **opt-in disclosure UX** (deferred until a UI exposes the mode choice; the env-var path needs no UX), tracked as future work — not a Stage 3 prerequisite.
