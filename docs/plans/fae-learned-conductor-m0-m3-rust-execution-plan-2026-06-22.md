...
- [ ] **M2 executor wiring** — connects BudgetGovernor + RoutingScorer + PII membrane into the conductor routing path. GATED on G-M2-spec + both WP reviewer passes + D-M2-1 resolution.
  ...
  - **Stage 3 (all-available default flip) — SEPARATE GATED COMMIT, blocked on:**
    1. **NOTE-2:** route `agent.run`/`agent.prompt` through the conductor — they currently bypass the §5 gate pipeline (pre-dating M2) and can reach cloud providers ungated. **(Next work — advisor-prioritized over reward/shadow: it's a real egress-coverage gap in the safety story that must close before any Stage 3 flip.)**
    2. The release-validation contract (`docs/checklists/app-release-validation.md`).
...
- [ ] **Reward aggregator (§7) + shadow router (§8) — DEFERRED from Stage 1:** narrowly scoped out to minimize the security-review surface. Land AFTER NOTE-2 closes (advisor: close the egress-coverage gap before adding the reward machinery).
