...
- [ ] **M2 executor wiring** — connects BudgetGovernor + RoutingScorer + PII membrane into the conductor routing path. GATED on G-M2-spec + both WP reviewer passes + D-M2-1 resolution.
  ...
  - **Stage 3 (all-available default flip) — SEPARATE GATED COMMIT, blocked on:**
    1. **NOTE-1:** replace mock provider pricing defaults (`main.rs` inserts 1/1 for any unpriced worker) with operator-configured real pricing before real adapters / all-available default.
    2. **NOTE-2:** route `agent.run`/`agent.prompt` through the conductor — they currently bypass the §5 gate pipeline (pre-dating M2) and can reach cloud providers ungated.
    3. The release-validation contract (`docs/checklists/app-release-validation.md`).
- [ ] **Reward aggregator (§7) + shadow router (§8) — DEFERRED from Stage 1:** narrowly scoped out to minimize the security-review surface. Next M2 sub-milestone after Stage 3 prerequisites are scoped.
