...
- [x] **Reward aggregator (§7) — COMMITTED** (`d1b4d79d`, dormant + test-covered): `crates/fae-daemon/src/conductor/reward.rs`. Four-signal aggregator (routing accuracy / user signal / outcome metrics / self-judgment); **F-10 enforced structurally** — self-judgment may never be the sole positive source (mutation-tested: `self_judgment_alone_cannot_produce_positive_reward`). Late-arriving feedback log (`conductor_feedback.jsonl`) + `FeedbackRecord`/`UserSignal` types + `append_feedback`/`read_feedback` store seam. 12 reward tests.
- [x] **Shadow router (§8) — COMMITTED** (`d1b4d79d`, dormant + test-covered): `crates/fae-daemon/src/conductor/shadow.rs`. **Structural no-egress** (holds only `Box<dyn ConductorRoutingPolicy>` + isolated store — no `CloudProvider`/`AcpAgentRunner` field can reach it; the `agent.session_start` lesson transferred: out-of-reach by type, not default). Decision-only candidate scoring; promotion flagging via `is_improvement` (NOT auto-deploy); `ShadowTurnRecord` persisted to isolated store. 4 shadow tests.
