...
- [ ] **M2 executor wiring** — connects BudgetGovernor + RoutingScorer + PII membrane into the conductor routing path. GATED on G-M2-spec + both WP reviewer passes + D-M2-1 resolution.
  ...
  - **Stage 3 (all-available default flip) — HELD (advisor-corrected 2026-06-23):** the "all-available destination default" was premised on the membrane "protecting users' data." Verified in source: the membrane catches **secrets/credentials only, NOT personal content** (health/address/location/family/finances/relationships). Default-on cloud egress would silently ship personal non-secret content to third parties against the product's local-only identity (ADR-001/003/007), with no consent UX. **Held until David re-decides the default privacy posture with this corrected premise.** Recommended: `pure-local`/`local-symphony` default, `all-available` as explicit disclosed opt-in. Full analysis: `docs/architecture/egress-scope-and-stage3-hold-2026-06-23.md`. NOTE-2 (egress-completeness) is CLOSED; the remaining gates are the privacy-posture decision + disclosure UX (if it flips) + release-validation contract.
...
- [ ] **Reward aggregator (§7) + shadow router (§8) — DEFERRED from Stage 1:** narrowly scoped out to minimize the security-review surface. **NOTE-2 is now CLOSED** — next M2 sub-milestone (non-egress, lower-risk; standard reviewer pass rather than red-team).
